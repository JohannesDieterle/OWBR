...
// TODO: check if global variable "context" works
...
function getCircle(p1, p2, p3) {
  ...
  if (Math.abs(A) < EPS) {
    return null;
    // throw new Error("Error: all points are in a line. No triangle can be formed!");
  }
...
export function calculate_elements(components) {
  ...
  for (let [i, component] of components.entries()) {
    ...
    if (circle == null) {
      throw new Error("Error: Supertriangle is a line!");
    }
    ...
    for (let point of component.points) {
      ...
      checkpoint: for (let k = newFreePoints.length - 1; k >= 0; k--) {
        for (let l = k - 1; l >= 0; l--) {
          if (
            Math.abs(newFreePoints[k].x - newFreePoints[l].x) < EPS &&
            Math.abs(newFreePoints[k].y - newFreePoints[l].y) < EPS
          ) {
      ...
      for (let k = 0; k < anglePoints.length; k++) {
        if (k == anglePoints.length - 1) {
          let circle = getCircle(point, anglePoints[k][1], anglePoints[0][1]);
  ...
  // Die Punkte mit gleichen Koordinaten in unterschiedlichen Bauteilen
  // werden durch den Masterpunkt ersetzt, damit keine Fehler bei der
  // Darstellung des Temperaturverlaufs und der Isothermen passieren
  const masterPoints = new Map();
  ...
  for (const component of components) {
    for (const element of component.elements) {
      for (let i = 0; i < element.points.length; i++) {
        ...
        const key = `${p.x},${p.y}`;
        if (masterPoints.has(key)) {
          // Ersetze durch das Master-Objekt
          element.points[i] = masterPoints.get(key);
        } else {
          masterPoints.set(key, p);
        }
      }
    }
  }
